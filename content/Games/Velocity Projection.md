...
[^1]: The spherical convex hull can be thought of as covering every point on the sphere you can express by normalising a weighted sum (in $\mathbb{R}^3$) of the input points, where the weights are non-negative. That weighted sum is known as a _conical combination_ of the inputs. The condition seen in Euclidean convex hulls that the weights sum to 1 is replaced by a condition that the end result has a _length_ of 1, which is achieved by the normalisation. 
